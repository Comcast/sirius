...
class LegacyUberStoreITest extends NiceTest with BeforeAndAfterAll {
  ...
  describe("During an interesting series of events...") {
    var uberStore = LegacyUberStore(tempDir.getAbsolutePath)
    ...
    it ("must cleanly transfer to a new handle") {
      ...
      uberStore.close()
      uberStore = LegacyUberStore(tempDir.getAbsolutePath)
  ...
  private def getAllEvents(uberStore: LegacyUberStore): List[OrderedEvent] = {
