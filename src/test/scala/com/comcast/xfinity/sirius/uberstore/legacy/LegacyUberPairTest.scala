...
object LegacyUberPairTest {

  def createMockedUpLog: (UberDataFile, SeqIndex, LegacyUberPair) = {
    val mockDataFile = mock(classOf[UberDataFile])
    val mockIndex = mock(classOf[SeqIndex])
    val underTest = new LegacyUberPair(mockDataFile, mockIndex)
    (mockDataFile, mockIndex, underTest)
  }
}

class LegacyUberPairTest extends NiceTest {

  import LegacyUberPairTest._
